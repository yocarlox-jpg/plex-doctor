...
VERSION="0.5.0"
SUMMARY_FILE="/tmp/emby-doctor-summary.txt"
FULL_LOG="/tmp/emby-doctor-full.log"
AUTO_INSTALL_DEPS=1
...
EMBY_STATUS="${WARN_ICON} No comprobado"
EMBY_DB_STATUS="${WARN_ICON} No comprobado"
...
No reinicia servicios, no borra archivos y no cambia configuración de Emby/rclone/discos.
EOF
}
...
filter_content_level_emby_noise() {
...
print_emby_error_summary() {
  local errors="$1"
  local h264 transcode_errors db_corruption db_busy

  if [[ -z "$errors" ]]; then
    printf "\nResumen de errores Emby:\n%s\n" "sin errores recientes encontrados"
    return
  fi

  h264="$(count_text_matches "$errors" 'non-existing PPS|decode_slice_header|no frame|invalid NAL|error while decoding|h264')"
  transcode_errors="$(count_text_matches "$errors" 'ffmpeg|transcod|TranscodingJob|PlaybackError')"
  ...
  db_busy="$(count_text_matches "$errors" 'database is locked|SQLITE_BUSY')"
  ...
  (( h264 > 0 )) && printf -- "- %s errores H264/decode. Normalmente apuntan a un vídeo/stream/cliente concreto, no a Emby entero.\n" "$h264"
  (( transcode_errors > 0 )) && printf -- "- %s señales de ffmpeg/transcode. Pueden ser normales si hay reproducción con conversión activa.\n" "$transcode_errors"

  if (( db_corruption + db_busy + h264 + transcode_errors == 0 )); then
  ...
  if (( db_corruption + db_busy > 0 )); then
    ...
    printf "%s\n" "$errors" | grep -Ei 'database disk image is malformed|database corruption|database is corrupt|SQLITE_CORRUPT|SQLITE_NOTADB|not a database|file is encrypted or is not a database|database is locked|SQLITE_BUSY' | tail -n 5
  fi
...
print_compact_systemctl_status() {
  printf "\n%s$ systemctl status emby-server --no-pager --lines=35%s\n" "${C_DIM}" "${C_RESET}"
  systemctl status emby-server --no-pager --lines=35 2>&1 \
    | sed -E 's/(ffmpeg|ffprobe|EmbyServer|emby-server).{120}.*/\1 ... [línea truncada]/'
}
...
collect_emby() {
  ...
  local emby_active transcoder_count port_listen log_dir db_file db_size emby_errors journal_errors
  ...
  db_busy_pattern='database is locked|SQLITE_BUSY'

  if have systemctl; then
    emby_active="$(systemctl is-active emby-server 2>/dev/null || true)"
    print_compact_systemctl_status
    case "$emby_active" in
      active) EMBY_STATUS="${OK_ICON} Running" ;;
      inactive|failed)
        EMBY_STATUS="${BAD_ICON} ${emby_active}"
        add_problem "$BAD_ICON" "emby-server está ${emby_active}"
        add_recommendation "revisar estado de emby-server"
        ;;
      *)
        EMBY_STATUS="${WARN_ICON} Estado desconocido"
        add_problem "$WARN_ICON" "no se pudo confirmar el estado de emby-server"
        ;;
    esac
  else
    EMBY_STATUS="${WARN_ICON} systemctl no disponible"
  fi

  subsection "Errores journalctl Emby últimas 24h"
  if have journalctl; then
    journal_errors="$(journalctl -u emby-server --since "24 hours ago" -p warning..alert --no-pager 2>/dev/null | tail -n 120 || true)"
    ...
    if printf "%s\n" "$journal_errors" | grep -Eiq 'timed out|SIGKILL|failed with result|failed mode'; then
      if [[ "$emby_active" == "active" ]]; then
        add_info "Emby tuvo timeout/fallo de parada anterior, pero ahora está running; revisar solo si coincide con cortes."
      else
        add_problem "$WARN_ICON" "Emby tuvo timeout/fallo de parada o arranque en las últimas 24h"
        add_recommendation "revisar journalctl de emby-server alrededor del último reinicio"
      fi
    fi
  else
  ...
  subsection "Procesos Emby"
  ps -eo pid,ppid,user,comm,args,%cpu,%mem --sort=-%cpu 2>/dev/null | grep -Ei '[e]mby|[f]fmpeg|[f]fprobe' | head -n 30 || true

  transcoder_count="$(count_processes 'ffmpeg.*(emby|transcod|transcoding|/var/lib/emby|/var/cache/emby)|emby.*ffmpeg')"
  print_kv "ffmpeg/transcode activos" "$transcoder_count"
  if (( transcoder_count >= 10 )); then
    TRANSCODER_STATUS="${BAD_ICON} ${transcoder_count} procesos activos"
    add_problem "$BAD_ICON" "transcodificación Emby activa con muchos procesos (${transcoder_count})"
    ...
    add_problem "$WARN_ICON" "transcodificación Emby activa con varios procesos (${transcoder_count})"
    add_recommendation "revisar transcodificaciones activas"
  elif (( transcoder_count >= 1 )); then
    TRANSCODER_STATUS="${OK_ICON} ${transcoder_count} procesos activos"
    add_info "Hay ${transcoder_count} procesos ffmpeg/transcode de Emby; normal si hay usuarios reproduciendo con conversión."
  fi

  subsection "Puertos Emby 8096/8920"
  if have ss; then
    ss -ltnp 2>/dev/null | awk 'NR==1 || /:8096|:8920/'
    port_listen="$(ss -ltn 2>/dev/null | grep -Ec ':8096|:8920' || true)"
  elif have netstat; then
    netstat -ltnp 2>/dev/null | awk 'NR==1 || /:8096|:8920/'
    port_listen="$(netstat -ltn 2>/dev/null | grep -Ec ':8096|:8920' || true)"
  else
    port_listen=0
    printf "%s%s ss/netstat no disponible%s\n" "$C_DIM" "$INFO_ICON" "$C_RESET"
  fi
  if (( port_listen < 1 )); then
    EMBY_STATUS="${BAD_ICON} Puertos 8096/8920 no escuchan"
    add_problem "$BAD_ICON" "Emby no parece estar escuchando en los puertos 8096/8920"
    add_recommendation "revisar si Emby está arrancado y escuchando en 8096 o 8920"
  fi

  log_dir="$(first_existing_dir "/var/lib/emby/logs" "/var/lib/emby-server/logs" "/opt/emby-server/logs")"
  subsection "Logs Emby"
  ...
  if [[ -d "$log_dir" ]]; then
    ...
    emby_errors="$(grep -RihE 'error|critical|exception|database is locked|SQLITE_BUSY|SQLITE_CORRUPT|SQLITE_NOTADB|not a database|file is encrypted or is not a database|database disk image is malformed|database corruption|database is corrupt|decode_slice_header|non-existing PPS|no frame|ffmpeg|transcod' "$log_dir"/*.log 2>/dev/null | filter_content_level_emby_noise | tail -n 80 || true)"
    print_emby_error_summary "$emby_errors"
    db_corruption_count="$(count_text_matches "$emby_errors" "$db_corruption_pattern")"
    db_busy_count="$(count_text_matches "$emby_errors" "$db_busy_pattern")"
    if (( db_corruption_count > 0 )); then
      EMBY_DB_STATUS="${BAD_ICON} Posible corrupción"
      add_problem "$BAD_ICON" "logs de Emby muestran posible corrupción de base de datos (${db_corruption_count} líneas)"
      add_recommendation "revisar base de datos de Emby y backups"
    elif (( db_busy_count >= 10 )); then
      EMBY_DB_STATUS="${WARN_ICON} DB ocupada"
      add_problem "$WARN_ICON" "Emby detecta DB ocupada/bloqueada repetidamente (${db_busy_count} líneas)"
      add_recommendation "revisar tareas pesadas de librería y logs de Emby relacionados con DB"
    elif (( db_busy_count > 0 )); then
      EMBY_DB_STATUS="${OK_ICON} Busy puntual"
      add_info "Emby detectó DB ocupada/bloqueada de forma puntual (${db_busy_count} líneas); suele ser normal durante escaneos o mucha actividad."
    fi
    if printf "%s\n" "$emby_errors" | grep -Eiq 'ffmpeg|transcod'; then
      add_info "Emby muestra actividad o errores de ffmpeg/transcode; revisar solo si coincide con cortes o load alto."
    fi
    if printf "%s\n" "$emby_errors" | grep -Eiq 'non-existing PPS|decode_slice_header|no frame|invalid NAL|error while decoding'; then
      add_info "Emby muestra errores H264/decode; normalmente apuntan a un archivo, stream o cliente concreto, no a un fallo global del servidor."
    fi
    if printf "%s\n" "$emby_errors" | grep -Eiq 'Session appears to have died|TranscodeOutputStream'; then
  ...
  db_file="$(first_existing_file "/var/lib/emby/data/library.db" "/var/lib/emby-server/data/library.db" "/opt/emby-server/data/library.db")"
  subsection "Base de datos Emby"
  print_kv "DB esperada" "$db_file"
  if [[ -f "$db_file" ]]; then
    db_size="$(du -h "$db_file" 2>/dev/null | awk '{print $1}')"
    print_kv "Tamaño DB" "${db_size:-desconocido}"
    if [[ "$EMBY_DB_STATUS" == "${WARN_ICON} No comprobado" ]]; then
      EMBY_DB_STATUS="${OK_ICON} Tamaño leído"
    else
      print_kv "Estado por logs" "$EMBY_DB_STATUS"
    fi
  else
    EMBY_DB_STATUS="${WARN_ICON} DB no encontrada"
    add_problem "$WARN_ICON" "no se encontró la DB de Emby en la ruta estándar"
  fi
}
...
collect_disks() {
  ...
  subsection "lsblk"
  if have lsblk; then
    if lsblk -o NAME,TYPE,SIZE,FSTYPE,LABEL,MOUNTPOINTS,MODEL,SERIAL >/tmp/emby-doctor-lsblk.$$ 2>/tmp/emby-doctor-lsblk-err.$$; then
      cat /tmp/emby-doctor-lsblk.$$
    elif lsblk -o NAME,TYPE,SIZE,FSTYPE,LABEL,MOUNTPOINT >/tmp/emby-doctor-lsblk.$$ 2>/tmp/emby-doctor-lsblk-err.$$; then
      cat /tmp/emby-doctor-lsblk.$$
    else
      lsblk 2>&1 || true
    fi
    rm -f /tmp/emby-doctor-lsblk.$$ /tmp/emby-doctor-lsblk-err.$$ 2>/dev/null || true
  else
...
collect_rclone() {
  ...
  while IFS= read -r mountpoint; do
    ...
    if ! safe_timeout 4 stat "$mountpoint" >/tmp/emby-doctor-stat.$$ 2>&1; then
      broken+="${mountpoint}: $(tr '\n' ' ' </tmp/emby-doctor-stat.$$ 2>/dev/null)\n"
      continue
    fi
  ...
  rm -f /tmp/emby-doctor-stat.$$ 2>/dev/null || true
...
collect_network() {
  ...
  if [[ -n "${gateway:-}" ]]; then
    if ping -c 2 -W 2 "$gateway" >/tmp/emby-doctor-ping-gw.$$ 2>&1; then
      ping_gw="ok"
    else
      ping_gw="fail"
    ...
    cat /tmp/emby-doctor-ping-gw.$$ 2>/dev/null || true
    rm -f /tmp/emby-doctor-ping-gw.$$ 2>/dev/null || true
  else
  ...
  if ping -c 2 -W 2 1.1.1.1 >/tmp/emby-doctor-ping-cloud.$$ 2>&1; then
    ping_cloud="ok"
  else
    ping_cloud="fail"
  ...
  cat /tmp/emby-doctor-ping-cloud.$$ 2>/dev/null || true
  rm -f /tmp/emby-doctor-ping-cloud.$$ 2>/dev/null || true

  subsection "DNS"
  cat /etc/resolv.conf 2>/dev/null || true
  if have getent; then
    if getent hosts emby.media >/dev/null 2>&1; then
      dns_ok="ok"
      print_kv "Resolución emby.media" "OK"
    else
      dns_ok="fail"
      NETWORK_STATUS="${WARN_ICON} DNS falla"
      add_problem "$WARN_ICON" "DNS no resuelve emby.media"
      add_recommendation "revisar DNS"
    fi
  fi

  subsection "Conexiones activas a puertos Emby"
  if have ss; then
    ss -tnp 2>/dev/null | awk 'NR==1 || /:8096|:8920/' | head -n 81
    conn_count="$(ss -tn 2>/dev/null | grep -Ec ':8096|:8920' || true)"
    print_kv "Conexiones 8096/8920" "$conn_count"
  fi
  ...
  if have ethtool; then
    ip -o link show up 2>/dev/null | awk -F': ' '{print $2}' | grep -Ev '^(lo|docker|veth|br-|virbr)' | while IFS= read -r iface; do
      [[ -z "$iface" ]] && continue
      printf "\n%s\n" "$iface"
      speed_output="$(ethtool "$iface" 2>/dev/null | grep -E 'Speed:|Duemby:|Link detected:' || true)"
...
probable_cause() {
  ...
  if printf "%s\n" "$joined" | grep -Eiq 'SMART alerta|errores I/O'; then
    ...
    echo "El servidor parece inestable por problema de mount/rclone o I/O, no por Emby directamente."
  elif printf "%s\n" "$joined" | grep -Eiq 'corrupción de base de datos'; then
    echo "El síntoma principal apunta a posible corrupción de la base de datos de Emby."
  elif printf "%s\n" "$joined" | grep -Eiq 'DB ocupada|DB.*bloqueada|base de datos'; then
    echo "El síntoma principal apunta a la DB de Emby ocupada o bloqueada por actividad interna."
  elif printf "%s\n" "$joined" | grep -Eiq 'transcodificación|transcode|ffmpeg'; then
    echo "La carga actual parece venir sobre todo de transcodificaciones activas en Emby."
  elif printf "%s\n" "$joined" | grep -Eiq 'RAM|Swap|OOM'; then
    echo "El servidor muestra presión de memoria; Emby puede estar afectado como consecuencia."
  elif printf "%s\n" "$joined" | grep -Eiq 'Gateway|DNS|internet|8096|8920'; then
    echo "La causa probable está en conectividad de red o exposición del puerto de Emby."
...
write_plain_diagnosis() {
  ...
  if printf "%s\n" "$joined" | grep -Eiq 'SMART alerta|errores I/O'; then
    ...
    echo "- ¿Es real?: sí. SMART o el kernel han devuelto errores reales; no es que Emby Doctor no pueda comprobarlo."
    echo "- Qué NO parece culpable: Emby, la DB de Emby, rclone o DNS no son la causa principal según esta ejecución."
    ...
    echo "- Qué NO parece culpable: Emby puede quedarse esperando datos, pero no tiene por qué ser el origen."
    ...
    echo "- Problema principal: posible corrupción de la DB de Emby."
    echo "- ¿Es real?: sí, aparece en logs de Emby; no significa reparar ya, significa hacer backup antes de tocar."
    echo "- Qué NO parece culpable: CPU, RAM, discos, rclone y red están OK en esta ejecución."
    echo "- Qué hacer ahora: guardar /tmp/emby-doctor-full.log, hacer backup de la DB y revisar las líneas exactas antes de mantenimiento."
    score_reason="Baja por señales de posible corrupción de DB."
  elif printf "%s\n" "$joined" | grep -Eiq 'DB ocupada|DB.*bloqueada'; then
    echo "- Problema principal: DB de Emby ocupada/bloqueada repetidamente."
    echo "- ¿Es real?: sí, Emby lo registró en logs, pero puede ser temporal durante escaneos, metadatos o mucha actividad."
    ...
    echo "- ¿Es real?: sí, hay procesos ffmpeg/transcode activos; puede ser normal si hay usuarios viendo contenido."
    ...
    echo "- Qué hacer ahora: repetir Emby Doctor cuando el problema esté ocurriendo."
  else
...
write_interpretation() {
  ...
  if printf "%s\n" "$joined" | grep -Eiq 'SMART alerta|errores I/O'; then
    echo "- Prioridad alta: almacenamiento físico o enlace SATA. SMART/I/O no es ruido de Emby; puede ser disco, cable, backplane, puerto o controladora."
  elif printf "%s\n" "$joined" | grep -Eiq 'mount roto|no responde correctamente|rclone|I/O wait'; then
    echo "- Prioridad alta: almacenamiento/mounts. Si rclone, FUSE o el disco se bloquean, Emby puede parecer culpable aunque solo esté esperando datos."
  fi
  if printf "%s\n" "$joined" | grep -Eiq 'transcodificación|transcode|ffmpeg'; then
    echo "- Hay transcodificación activa. Esto puede explicar load alto o CPU alta, sobre todo con subtítulos quemados, audio EAC3/DTS o clientes poco compatibles."
  fi
  if printf "%s\n" "$joined" | grep -Eiq 'corrupción de base de datos'; then
    echo "- La DB de Emby muestra señales compatibles con corrupción. Prioridad: backup de la DB antes de cualquier reparación o limpieza."
  elif printf "%s\n" "$joined" | grep -Eiq 'DB ocupada|DB.*bloqueada|base de datos'; then
    echo "- La DB de Emby aparece ocupada/bloqueada. Esto suele pasar con escaneos, metadatos, tareas programadas o mucha actividad; solo preocupa si se repite y coincide con lentitud/cortes."
  fi
  if printf "%s\n" "$joined" | grep -Eiq 'timeout/fallo de parada|SIGKILL|failed'; then
    echo "- Emby tuvo problemas al parar/arrancar recientemente. Eso puede dejar sesiones, cache o procesos en estado raro aunque ahora el servicio aparezca running."
  fi
  if printf "%s\n" "$joined" | grep -Eiq 'RAM|Swap|OOM'; then
    echo "- Hay presión de memoria. Si aparece OOM killer, cualquier síntoma posterior de Emby puede ser consecuencia de falta de RAM."
  fi
  if printf "%s\n" "$joined" | grep -Eiq 'Gateway|DNS|internet|8096|8920'; then
    echo "- Hay señales de red o puertos 8096/8920. Si la red falla, el servidor puede estar sano pero inaccesible para clientes externos."
  fi
  if printf "%s\n" "$joined" | grep -Eiq 'thermal|Temperatura'; then
    echo "- Hay señal térmica. Si el equipo reduce frecuencia por temperatura, Emby transcodificando será lo primero que se degrade."
  fi
}
...
write_action_plan() {
  ...
  echo "1. Confirmar versión: bash emby-doctor.sh --version."

  if printf "%s\n" "$joined" | grep -Eiq 'SMART alerta|errores I/O'; then
    echo "2. Tratar almacenamiento como prioridad: backup, revisar SMART completo y comprobar cable/backplane/puerto antes de culpar a Emby."
  elif printf "%s\n" "$joined" | grep -Eiq 'mount roto|no responde correctamente|rclone|I/O wait'; then
    echo "2. Revisar almacenamiento antes que Emby: comprobar que los mounts rclone responden, mirar logs de rclone y confirmar que el disco/cache no está saturado."
  elif printf "%s\n" "$joined" | grep -Eiq 'transcodificación|transcode|ffmpeg'; then
    echo "2. Revisar sesiones activas en Emby: identificar usuarios/dispositivos que transcodifican, subtítulos quemados y audios que obligan a convertir."
  elif printf "%s\n" "$joined" | grep -Eiq 'corrupción de base de datos'; then
    echo "2. Hacer backup de la DB de Emby y revisar las líneas exactas del full log antes de cualquier mantenimiento."
  elif printf "%s\n" "$joined" | grep -Eiq 'DB ocupada|base de datos'; then
    echo "2. Revisar qué tarea de Emby estaba activa: escaneo, metadatos, detección de intros/créditos o muchos usuarios."
  else
    echo "2. Atacar primero los problemas rojos del resumen; si solo hay amarillos, repetir prueba durante el fallo real."
  fi

  if printf "%s\n" "$joined" | grep -Eiq 'timeout/fallo de parada|SIGKILL|failed'; then
    echo "3. Revisar el tramo del reinicio: journalctl -u emby-server --since 'YYYY-MM-DD HH:MM' --no-pager para entender por qué Emby no cerró limpio."
  else
    echo "3. Guardar /tmp/emby-doctor-full.log y comparar con una segunda ejecución cuando el problema esté ocurriendo."
  fi
